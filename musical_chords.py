...
while(True):
    for song_name in SONGS:
        ...
        for _ in range(2):
            for chord in song:
                print(MIDI_NOTE_NAMES[chord[0]%12], chord[1])
                offset = chord[0]
    ...
    print("all chords:")
    for scale_name in CHORDS:
        scale = CHORDS[scale_name]
        print(scale_name, scale)
        notes = [STARTING_NOTE+sum(scale[:i]) for i in range(len(scale)+1)]
        notes += [STARTING_NOTE+12+sum(scale[:i+1]) for i in range(len(scale))]
        player.note_on(notes[0]-24, 127)
        for note in (notes+notes[::-1][1:])[:12]:
            player.note_on(note, 127)
            time.sleep(sixteenth_note)
